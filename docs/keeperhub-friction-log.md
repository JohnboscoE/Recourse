...
| 10 | 2026-07-27 | x402 / errors | A clock-skew failure that says so | A host clock 2h32m slow made every payment fail with `KeeperHubError: Timestamp outside replay window` / `code: HMAC_INVALID`. The message names the symptom but not the cause, and "HMAC_INVALID" points a builder at their credentials — the one thing that was fine. Cost ~20 min. | Suggestion: when the HMAC timestamp is outside the window, say so explicitly and include the server's time and the observed skew. The server knows both. Worked around by signing under the server's `Date` header (`agents/src/x402.ts`). | high |
| 11 | 2026-07-27 | Marketplace | Paid listings that work | Paid for two listings at $0.01 each. **Both charged successfully and neither returned a result.** `wallet-snapshot-base` → `Unresolved template reference(s): {{@trigger:Manual.borrower}}`; on a corrected body → `Protocol "aave-v3" contract "pool" is not deployed on network "undefined"`. `microtip` → 503. Money is taken before the workflow is known to run, and there is no refund path. | Suggestion: validate listings before they can be listed (`validate_workflow` already exists — gate `list_workflow` on it), and/or don't settle payment until the execution reaches a non-error terminal state. | high |
| 12 | 2026-07-27 | Marketplace / API | One request shape for one operation | The MCP tool `call_workflow` takes `{slug, inputs:{...}}`, but the HTTP endpoint behind it (`POST /api/mcp/workflows/{slug}/call`) wants the fields at the **top level** of the body. Nesting them under `inputs` is accepted, charged for, and then fails inside the workflow with a confusing template error. The only hint is the `extensions.bazaar.schema` (`input.body.<field>`) buried in the 402. | Suggestion: accept both shapes, or reject an unrecognised body *before* taking payment. | med |
